...
class BrokerFaxRepository {
  static ALL_FIELDS = ['fax'];
  ...
  static async create(data, options: IRepositoryOptions) {
    ...
    await options.database.broker_fax.create(
      {
  ...
  static async destroyByBroker(
    ...
    await options.database.broker_fax.destroy({
      where: {
        broker_id: id,
      },
      transaction,
  ...
  static async findAndCountAll(
    ...
    let { rows, count } =
      await options.database.broker_fax.findAndCountAll({
        where,
        include,
...
export default BrokerFaxRepository;
