...
class BrokerRegulatoryAuthorityRepository {
  ...
  static async create(data, options: IRepositoryOptions) {
    ...
    await options.database.broker_regulatory_authority.create(
      {
  ...
  static async destroyByBroker(
    ...
    await options.database.broker_regulatory_authority.destroy(
      {
        where: {
          broker_id: id,
        },
  ...
  static async findAndCountAll(
    ...
    let { rows, count } =
      await options.database.broker_regulatory_authority.findAndCountAll(
        {
          where,
          include,
...
export default BrokerRegulatoryAuthorityRepository;
