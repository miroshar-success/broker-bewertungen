...
class BrokerBankRepository {
  ...
  static async create(data, options: IRepositoryOptions) {
    ...
    await options.database.broker_bank.create(
      {
  ...
  static async destroyByBroker(
    ...
    await options.database.broker_bank.destroy({
      where: {
        broker_id: id,
      },
      transaction,
  ...
  static async findAndCountAll(
    ...
    let { rows, count } =
      await options.database.broker_bank.findAndCountAll({
        where,
        include,
...
export default BrokerBankRepository;
